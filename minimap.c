...
void    put_minimap(t_ptr *ptr)
{
	...
	while (y < MAP_W)
	{
		...
		while (x < MAP_H)
		{
            if (x + x_map >=  0 && x + x_map < ptr->parse.x * MAP_SCALE && y + y_map >= 0 && y + y_map < ptr->parse.y * MAP_SCALE)
                my_mlx_pixel_put(&ptr->img3d, x, y, return_color_to_minimap(ptr, y + y_map, x + x_map));
            else
